...
const STARTER_WORDS = new Set([
  "i","you","we","hey","that","its","this","honestly","love","feel",
  "think","know","maybe","always","never","every","life","just","so",
  "omg","aww","wow","okay","well","hope","trust","real","tbh","ngl"
]);

function qualityCheck(words) {
  if (words.length < 5) return false;
  // Must start with a valid starter word
  if (!STARTER_WORDS.has(words[0].toLowerCase())) return false;
  // No consecutive duplicate words
  for (let i = 0; i < words.length - 1; i++) {
    if (words[i].toLowerCase() === words[i + 1].toLowerCase()) return false;
  }
  // No single-character words except "I"
  if (words.some(w => w.length === 1 && w.toLowerCase() !== "i")) return false;
  return true;
}
...
function generateFromChain(chain, input, maxWords = 22) {
  ...
  // Filter to starter-word keys only
  const starterKeys = keys.filter(k => {
    const first = k.startsWith("__bi__")
      ? k.replace("__bi__ ", "").split(" ")[0]
      : k.split(" ")[0];
    return STARTER_WORDS.has(first.toLowerCase());
  });

  for (let attempt = 0; attempt < 12; attempt++) {
    // Prefer starter keys, fall back to any key
    const seedKey = starterKeys.length > 0
      ? (Math.random() < 0.8
          ? starterKeys[Math.floor(Math.random() * starterKeys.length)]
          : findSeedKey(input, chain))
      : findSeedKey(input, chain);
    ...
    if (qualityCheck(result)) return capitalize(result.join(" "));
  }
