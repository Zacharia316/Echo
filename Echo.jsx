...
export default function EchoApp() {
  ...
  const generateImage = async (prompt) => {
    ...
    setThinking(true);
    try {
      const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=flux&width=512&height=512&nologo=true&key=${POLLINATIONS_KEY}`;
      await fetch(url);
      const done = [...base,{ f:"e",t:"__img__",img:url,id:loadId,ts:Date.now() }];
      pushMsgs(done);
      setMessages(done);
    } catch {
