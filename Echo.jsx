...
async function webSearch(query) {
  try {
    const res = await fetch(`https://apiskeith.top/ai/searchai?query=${encodeURIComponent(query)}`);
    if (!res.ok) throw new Error();
    const data = await res.json();
    const result = data?.result || data?.response || data?.answer || data?.message || "";
...
async function webFetch(url) {
  try {
    const res = await fetch(`https://apiskeith.top/ai/searchai?query=${encodeURIComponent(url)}`);
    if (!res.ok) throw new Error();
    const data = await res.json();
    const result = data?.result || data?.response || data?.answer || data?.message || "";
    if (result && result.length > 10) return result;
...
const generateImage = async (prompt) => {
  ...
  const send = async (overrideText) => {
    ...
    if (nameR) { pushMsgs([...messages,{f:"u",t:txt,ts:Date.now()},{f:"e",t:nameR,sys:true,ts:Date.now()}]); tts(nameR,ttsOn); return; }
    const memR = checkMemory(txt);
    if (memR) { pushMsgs([...messages,{f:"u",t:txt,ts:Date.now()},{f:"e",t:memR,sys:true,ts:Date.now()}]); tts(memR,ttsOn); return; }
...
const handleImageUpload = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      const userMsg = { f:"u", t:"📷 [Sent an image]", ts:Date.now(), imgUrl: ev.target.result };
      ...
      try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 1000,
            system: buildSystem(aiName, mode, userName, memories, trainedData, emotion),
            messages: [{
              role: "user",
              content: [
                { type: "image", source: { type: "base64", media_type: mediaType, data: base64 } },
                { type: "text", text: "Describe or respond to this image." }
              ]
            }]
          })
        });
        const data = await response.json();
        const reply = data?.content?.[0]?.text || "I can see the image! Interesting 🌸";
        const aiMsg = { f:"e", t:reply, id:Date.now(), ts:Date.now(), src:"ai" };
        const newM = [...withUser, aiMsg];
        pushMsgs(newM);
        tts(reply, ttsOn);
      } catch {
        const errMsg = { f:"e", t:"Couldn't analyze that image right now 🌸", id:Date.now(), ts:Date.now(), src:"ai" };
        pushMsgs([...withUser, errMsg]);
      } finally {
