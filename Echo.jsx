...
// SVG Icons
const IcoBack = ({ color = "rgba(240,234,248,0.5)" } = {}) => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>;
const IcoGear = ({ color = "rgba(240,234,248,0.5)" } = {}) => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>;
...
const IcoSpeaker = ({ muted, color = "rgba(240,234,248,0.5)" }) => muted
  ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>
  : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>;
...
const IcoMic = ({ active, activeColor = "#e8a0b4", color = "#9a7e6a" }) => <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? activeColor : "none"} stroke={active ? activeColor : color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>;
const IcoImage = ({ color = "rgba(240,234,248,0.5)" } = {}) => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>;
const IcoPlus = ({ color = "rgba(240,234,248,0.5)" } = {}) => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>;
const IcoCopy = ({ color = "rgba(240,234,248,0.5)" } = {}) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>;
function IcoKey({ color = "rgba(240,234,248,0.5)" } = {}) {
  return (
...
const [activeEnhance, setActiveEnhance] = useState(() => ls("echo_active_enhance", null));
  ...
  const [dark, setDark] = useState(() => ls("echo_dark", false));
  ...
  const T = dark ? {
    ...
    text:       "#f0eaf8",
    textSub:    "rgba(240,234,248,0.45)",
    textMuted:  "rgba(240,234,248,0.25)",
    inputBg:    "rgba(255,255,255,0.06)",
    inputColor: "#f0eaf8",
    inputPh:    "rgba(240,234,248,0.3)",
    shadow1:    "6px 6px 16px rgba(0,0,0,0.55)",
    shadow2:    "-3px -3px 10px rgba(255,255,255,0.03)",
    msgUser:    `linear-gradient(135deg,${pal.b}ee,${pal.c}cc)`,
    msgUserTxt: "#fff",
    msgAi:      "rgba(255,255,255,0.05)",
    msgAiTxt:   "#f0eaf8",
    msgAiBorder:"rgba(255,255,255,0.08)",
    barBg:      "rgba(13,13,22,0.88)",
    modeBg:     "rgba(13,13,22,0.7)",
    scrollbar:  "rgba(255,255,255,0.1)",
    placeholderTxt: "rgba(240,234,248,0.3)",
  } : {
    bg:         "#e8e8f0",
    bg2:        "#e0e0ea",
    surface:    "#e8e8f0",
    ...
    inputColor: "#2a2a3a",
    inputPh:    "#a0a0b8",
    shadow1:    "6px 6px 14px #c8c8d8",
    shadow2:    "-4px -4px 10px #ffffff",
    msgUser:    `linear-gradient(135deg,${pal.b},${pal.c})`,
    msgUserTxt: "#fff",
    msgAi:      "rgba(255,255,255,0.65)",
    msgAiTxt:   "#2a2a3a",
    msgAiBorder:"rgba(255,255,255,0.9)",
    barBg:      "rgba(232,232,240,0.92)",
    modeBg:     "rgba(232,232,240,0.85)",
    scrollbar:  `${pal.b}55`,
    placeholderTxt: "#a0a0b8",
  };

  const GF = `@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=Lato:wght@300;400&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{background:${T.bg}}input,textarea{color:${T.inputColor}!important;background:${T.inputBg}!important;border-color:${T.border}!important}input::placeholder,textarea::placeholder{color:${T.inputPh}!important}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:${T.scrollbar};border-radius:4px}`;
  const pf = { fontFamily:"'Playfair Display',serif" };
  const lf = { fontFamily:"'Lato',sans-serif" };
  const BG = {
    background: dark
      ? `radial-gradient(ellipse at 20% 20%,${pal.b}28 0%,transparent 55%),radial-gradient(ellipse at 80% 80%,${pal.c}20 0%,transparent 55%),linear-gradient(160deg,#0d0d16,#12121e,#0f0f1a)`
      : `radial-gradient(ellipse at 15% 15%,${pal.b}30 0%,transparent 50%),radial-gradient(ellipse at 85% 85%,${pal.c}25 0%,transparent 50%),linear-gradient(160deg,#e8e8f0,#e0e0ea,#e8e8f0)`,
    minHeight:"100vh"
  };
  const neuCard = {
    background: T.surface,
    borderRadius: 18,
    boxShadow: `${T.shadow1},${T.shadow2}`,
  ...
  const ThemeToggle = ({ style = {} }) => (
    <button
      onClick={() => { setDark(d => { lsSave("echo_dark", !d); return !d; }); }}
      style={{
        width: 38, height: 22, borderRadius: 11,
        background: dark ? `linear-gradient(135deg,${pal.b},${pal.c})` : `${T.shadow1 ? "#c8c8d8" : "#ccc"}`,
        border: "none", cursor: "pointer", position: "relative",
        boxShadow: `${T.shadow1},${T.shadow2}`, transition: "all 0.3s",
        flexShrink: 0,
        ...style,
      }}
    ...
    >
      <div style={{
        ...
        background: dark ? "#fff" : pal.b,
        transition: "left 0.25s", boxShadow: "0 1px 4px rgba(0,0,0,0.2)",
        display: "flex", alignItems: "center", justifyContent: "center",
      }}>
        {dark
          ? <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={pal.b} strokeWidth="2.5" strokeLinecap="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
          : <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>
        }
      </div>
    </button>
  );
...
const generateImage = async (prompt) => {
  ...
  if (pendingImages.length > 0) {
    ...
    try {
      const analyzeImage = async (imgs, caption, em) => {
  const sys = buildSystem(aiName, mode, userName, memories, trainedData, em);
  const key = activeChat ? apiKeys.chat[activeChat] : "";
  const reply = await analyzeImage(imgs, caption, em);
const final = reply || "Couldn't analyze that image 🌸 Make sure a vision API is active in the hub 🔑";
pushMsgs([...withUser, { f:"e", t:final, id:Date.now(), ts:Date.now(), src:"ai" }]);
tts(final, ttsOn);

  try {
    // CLAUDE
    if (activeChat === "claude" && key) {
    ...
    // GEMINI
    if (activeChat === "gemini" && key) {
      const parts = [
        ...imgs.map(img => ({ inline_data: { mime_type: img.mediaType, data: img.base64 }})),
        { text: caption }
      ];
      ...
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: [
              ...imgs.map(img => ({ type: "image_url", image_url: { url: `data:${img.mediaType};base64,${img.base64}` }})),
              { type: "text", text: caption }
            ]}
          ]
        })
...
// TC Screen
  if (screen === "tc") return (
    <div style={{ ...BG,...lf,display:"flex",alignItems:"center",justifyContent:"center",padding:28,position:"relative" }}>
      <style>{GF}</style>
      <div style={{ position:"absolute",top:18,right:18,display:"flex",alignItems:"center",gap:8 }}>
        <span style={{ fontSize:10,color:T.textMuted,letterSpacing:1 }}>{dark?"DARK":"LIGHT"}</span>
        <ThemeToggle/>
      </div>
      <div style={{ maxWidth:340,width:"100%" }}>
        <div style={{ textAlign:"center",marginBottom:24 }}>
          <div style={{ width:56,height:56,borderRadius:18,background:T.surface,boxShadow:`${T.shadow1},${T.shadow2}`,margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center" }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={pal.b} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          </div>
          <h2 style={{ ...pf,fontSize:22,color:T.text }}>Before you meet {aiName}</h2>
          <p style={{ fontSize:12,color:T.textSub,marginTop:6 }}>Please read and accept our terms</p>
        </div>
        <div style={{ ...neuCard,padding:18,fontSize:12,color:T.textSub,lineHeight:1.9,marginBottom:20 }}>
          <p style={{ ...pf,fontWeight:"bold",marginBottom:8,color:T.text }}>Terms & Conditions — {aiName} by Luminar Inc</p>
        ...
        <button onClick={()=>{localStorage.setItem("echo_tc","1");setScreen("home");}} style={{ width:"100%",padding:"14px 0",borderRadius:30,background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:15,...pf,cursor:"pointer",boxShadow:`0 4px 20px ${pal.b}44,${T.shadow2}` }}>
          I Agree — Meet {aiName}
        </button>
        <p style={{ textAlign:"center",fontSize:10,color:T.textMuted,marginTop:12,letterSpacing:2 }}>{aiName.toUpperCase()} BY LUMINAR INC</p>
      </div>
    </div>
  );
  ...
  if (screen === "home") return (
    <div style={{ ...BG,...lf,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,minHeight:"100vh",position:"relative" }}>
      <style>{GF}</style>
      <div style={{ position:"absolute",top:18,right:18,display:"flex",alignItems:"center",gap:8 }}>
        <span style={{ fontSize:10,color:T.textMuted,letterSpacing:1 }}>{dark?"DARK":"LIGHT"}</span>
        <ThemeToggle/>
      ...
      <div style={{ textAlign:"center",maxWidth:340,width:"100%" }}>
        ...
        <div style={{ marginTop:28 }}>
          <h1 style={{ ...pf,fontSize:44,fontWeight:700,color:T.text,letterSpacing:2 }}>{aiName}</h1>
          <p style={{ fontSize:12,color:pal.b,letterSpacing:3,textTransform:"uppercase",marginTop:4 }}>by Luminar Inc</p>
        </div>
        <p style={{ ...pf,fontSize:22,color:T.text,marginTop:20,lineHeight:1.5 }}>
          {userName ? `${getGreeting()}, ${userName}! ${MOOD_EMOJI[emotion]}` : `How can I help you today?`}
        </p>
        <p style={{ color:T.textSub,fontSize:13,marginTop:10,lineHeight:1.7 }}>Your personal AI — she learns from you, grows with you, remembers you.</p>
        ...
        <button onClick={()=>{
          ...
          setScreen("chat");
        }} style={{ marginTop:24,width:"100%",padding:"15px 0",borderRadius:30,background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:16,...pf,letterSpacing:1,cursor:"pointer",boxShadow:`0 4px 20px ${pal.b}44,${T.shadow2}` }}>
          Start Talking
        </button>
        <button onClick={()=>setScreen("settings")} style={{ marginTop:14,background:"none",border:"none",color:T.textSub,fontSize:13,cursor:"pointer",letterSpacing:1,display:"flex",alignItems:"center",gap:6,margin:"14px auto 0" }}>
          <IcoGear color={T.textSub}/> Settings &amp; Training
        </button>
      </div>
    </div>
  );
  ...
  if (screen === "chat") return (
    <div style={{ height:"100vh",background:dark?`radial-gradient(ellipse at 20% 20%,${pal.b}22 0%,transparent 55%),radial-gradient(ellipse at 80% 80%,${pal.c}18 0%,transparent 55%),linear-gradient(160deg,#0d0d16,#12121e,#0f0f1a)`:`radial-gradient(ellipse at 15% 10%,${pal.b}25 0%,transparent 50%),radial-gradient(ellipse at 85% 90%,${pal.c}20 0%,transparent 50%),linear-gradient(160deg,#e8e8f0,#e0e0ea,#e8e8f0)`,display:"flex",flexDirection:"column",position:"relative",...lf }}>
      <style>{GF}</style>
      {/* Header */}
      <div style={{ padding:"12px 16px",background:T.barBg,backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,boxShadow:dark?`0 2px 24px rgba(0,0,0,0.5)`:`0 2px 16px #c8c8d844` }}>
        <button onClick={()=>setScreen("home")} style={{ background:"none",border:"none",cursor:"pointer" }}><IcoBack color={T.textSub}/></button>
        <div onClick={tapOrb} style={{ width:34,height:34,borderRadius:"50%",background:`radial-gradient(circle at 35% 35%,${pal.a},${pal.b})`,boxShadow:`0 2px 12px ${pal.b}66,${T.shadow2}`,flexShrink:0,cursor:"pointer",transition:"background 0.5s" }}/>
        <div style={{ flex:1 }}>
          <div style={{ ...pf,fontSize:16,color:T.text,fontWeight:700 }}>{aiName}</div>
          <div style={{ fontSize:10,color:pal.b }}>{mode} · {emotion!=="neutral"?emotion:"listening"} {MOOD_EMOJI[emotion]}</div>
        </div>
        <ThemeToggle/>
        <button onClick={()=>{setTtsOn(v=>{lsSave("echo_tts",!v);return !v;});}} style={{ background:"none",border:"none",cursor:"pointer",padding:6 }}><IcoSpeaker muted={!ttsOn} color={T.textSub}/></button>
        <button onClick={()=>setScreen("apikeys")} style={{background:"none",border:"none",cursor:"pointer",padding:6}}>
          <IcoKey color={T.textSub}/>
        </button>
        <button onClick={()=>setScreen("settings")} style={{ background:"none",border:"none",cursor:"pointer",padding:6 }}><IcoGear color={T.textSub}/></button>
      </div>
      {/* Mode bar */}
      <div style={{ padding:"8px 14px",background:T.modeBg,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",borderBottom:`1px solid ${T.border}`,display:"flex",gap:6 }}>
        {MODES.map(m=>(
          <button key={m} onClick={()=>{setMode(m);lsSave("echo_mode",m);}} style={{ padding:"5px 14px",borderRadius:12,border:`1px solid ${mode===m?pal.b+"66":T.border}`,background:mode===m?`${pal.b}22`:dark?"rgba(255,255,255,0.04)":T.surface2,color:mode===m?pal.b:T.textSub,fontSize:11,cursor:"pointer",boxShadow:mode===m?`0 2px 12px ${pal.b}33,${T.shadow2}`:`${T.shadow1},${T.shadow2}`,transition:"all 0.2s" }}>{m}</button>
        ))}
      </div>
      <div ref={chatRef} onScroll={handleScroll} style={{ flex:1,overflowY:"auto",padding:"16px 12px",display:"flex",flexDirection:"column",gap:8,position:"relative" }}>
        {messages.map((msg,i)=>(
          ...
          >
            <div style={{ maxWidth:"78%",padding:"11px 15px",borderRadius:msg.f==="u"?"20px 20px 4px 20px":"20px 20px 20px 4px",background:msg.f==="u"?T.msgUser:T.msgAi,backdropFilter:msg.f==="e"?"blur(16px)":"none",WebkitBackdropFilter:msg.f==="e"?"blur(16px)":"none",color:msg.f==="u"?T.msgUserTxt:T.msgAiTxt,fontSize:14,lineHeight:1.65,boxShadow:msg.f==="u"?`0 4px 20px ${pal.b}55,${T.shadow2}`:`${T.shadow1},${T.shadow2}`,border:msg.f==="e"?`1px solid ${T.msgAiBorder}`:"none",whiteSpace:"pre-line",position:"relative" }}>
              {!msg.sys && (
                <div style={{ fontSize:9,color:msg.src==="markov"?"#c4a8d4":msg.f==="u"?`rgba(255,255,255,0.65)`:`${pal.b}88`,letterSpacing:1.5,marginBottom:4,textTransform:"uppercase" }}>
...
{msg.t && (() => {
  ...
  if (htmlMatch || isFullDoc || jsxMatch) {
    ...
    return (
      <div>
        ...
        <div style={{marginTop:8,background:T.inputBg,borderRadius:12,overflow:"hidden",border:`1px solid ${T.border}`}}>
          <div style={{padding:"6px 12px",fontSize:10,color:T.textSub,letterSpacing:1,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
  ...
  return <span style={{whiteSpace:"pre-line"}}>{msg.t}</span>;
              ...
              {msg.imgUrl && msg.f === "e" && (
                <div style={{ marginTop:8 }}>
                  ...
                  <a href={msg.imgUrl} download="echo-image.jpg" target="_blank" style={{ display:"flex",alignItems:"center",justifyContent:"center",marginTop:6,gap:4,color:pal.b,textDecoration:"none",fontSize:11 }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={pal.b} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Save Image
                  </a>
                </div>
              )}
      ...
      {showScrollBtn && (
        <button onClick={()=>endRef.current?.scrollIntoView({behavior:"smooth"})}
          style={{ position:"absolute",bottom:90,right:16,width:38,height:38,borderRadius:"50%",background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:18,cursor:"pointer",boxShadow:`0 2px 12px ${pal.b}66,${T.shadow2}`,zIndex:10 }}>
          ↓
        </button>
      )}
      ...
      {/* Plus menu */}
      {showPlusMenu && (
        <div style={{ position:"absolute",bottom:80,left:14,background:T.barBg,backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderRadius:16,border:`1px solid ${T.border}`,boxShadow:`${T.shadow1},${T.shadow2}`,padding:"8px 0",zIndex:20,minWidth:160 }}>
          <button onClick={startListening} style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 18px",background:"none",border:"none",cursor:"pointer",width:"100%",color:T.text,fontSize:14 }}>
            <IcoMic active={isListening}/> {isListening ? "Listening..." : "Voice Input"}
          </button>
          <div style={{ height:1,background:T.border,margin:"0 12px" }}/>
          <button onClick={()=>imgInputRef.current?.click()} style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 18px",background:"none",border:"none",cursor:"pointer",width:"100%",color:T.text,fontSize:14 }}>
            <IcoImage/> Send Image
          </button>
        </div>
      )}
      ...
      {/* Input bar */}
      <div style={{ padding:"10px 14px 14px",background:T.barBg,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderTop:`1px solid ${T.border}` }}>
        <div style={{ background:T.surface,borderRadius:20,border:`1px solid ${T.border}`,boxShadow:`${T.shadow1},${T.shadow2}`,padding:"10px 14px",display:"flex",flexDirection:"column",gap:8 }}>
          {pendingImages.length > 0 && (
            <div style={{ display:"flex",gap:6,flexWrap:"wrap",paddingBottom:4 }}>
              {pendingImages.map(img => (
                <div key={img.id} style={{ position:"relative" }}>
                  <img src={img.preview} style={{ width:56,height:56,borderRadius:10,objectFit:"cover",border:`1.5px solid ${T.border}` }}/>
                  <button onClick={()=>setPendingImages(prev=>prev.filter(i=>i.id!==img.id))} style={{ position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:"50%",background:pal.b,border:"none",cursor:"pointer",color:"white",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button>
                </div>
              ))}
            </div>
          )}
          <textarea
            ...
            rows={1}
            style={{ width:"100%",border:"none",outline:"none",background:"transparent",fontSize:14,color:T.text,resize:"none",lineHeight:1.5,maxHeight:120,overflowY:"auto",fontFamily:"'Lato',sans-serif" }}
            onInput={e => {
          ...
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            <button onClick={()=>setShowPlusMenu(v=>!v)} style={{ width:32,height:32,borderRadius:"50%",background:T.surface,border:`1.5px solid ${T.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`${T.shadow1},${T.shadow2}` }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.textSub} strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            </button>
            <div style={{ fontSize:10,color:T.textMuted }}>↵ send · shift+↵ newline</div>
            <button onClick={()=>send()} disabled={thinking||(!input.trim()&&!pendingImages.length)} style={{ width:36,height:36,borderRadius:"50%",background:thinking||(!input.trim()&&!pendingImages.length)?T.surface:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",cursor:thinking||(!input.trim()&&!pendingImages.length)?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"white",boxShadow:!thinking&&(input.trim()||pendingImages.length)?`0 2px 12px ${pal.b}44,${T.shadow2}`:T.shadow1,transition:"all 0.2s",flexShrink:0 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={thinking||(!input.trim()&&!pendingImages.length)?T.textSub:"white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
          </div>
        </div>
      </div>
  ...
  if (screen === "settings") return (
    <div style={{ height:"100vh",overflowY:"auto",...BG,padding:24,...lf }}>
      <style>{GF}</style>
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20 }}>
        <button onClick={()=>setScreen("home")} style={{ background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:6,color:T.textSub,fontSize:14 }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.textSub} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg> Back
        </button>
      ...
      <h2 style={{ ...pf,fontSize:26,color:T.text,marginBottom:4 }}>Settings</h2>
      <p style={{ fontSize:12,color:T.textSub,marginBottom:24 }}>Customize & train {aiName}</p>

      {/* AI Name */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:6,letterSpacing:1,textTransform:"uppercase" }}>AI Name</div>
        <p style={{ fontSize:11,color:T.textSub,marginBottom:10 }}>Rename {aiName} — she'll respond to it everywhere</p>
        <input defaultValue={aiName} onBlur={e=>{const v=e.target.value.trim()||"Echo";setAiName(v);lsSave("echo_name",v);}} placeholder="Echo" style={{ width:"100%",padding:"10px 14px",borderRadius:10,border:`1.5px solid ${T.border}`,fontSize:14,color:T.text,outline:"none",background:T.inputBg,boxShadow:`inset ${T.shadow1}` }}/>
      </div>

      {/* Personality */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:10,letterSpacing:1,textTransform:"uppercase" }}>Personality Mode</div>
        <div style={{ display:"flex",gap:8 }}>
          {MODES.map(m=><button key={m} onClick={()=>{setMode(m);lsSave("echo_mode",m);}} style={{ flex:1,padding:"9px 0",borderRadius:10,border:`1.5px solid ${mode===m?pal.b+"66":T.border}`,background:mode===m?`${pal.b}22`:T.inputBg,color:mode===m?pal.b:T.textSub,fontSize:12,cursor:"pointer",boxShadow:mode===m?`0 2px 12px ${pal.b}33`:`${T.shadow1},${T.shadow2}`,transition:"all 0.2s" }}>{m}</button>)}
        </div>
      </div>

      {/* TTS */}
      <div style={{ ...neuCard,padding:18,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div>
          <div style={{ fontSize:13,color:T.text,fontWeight:"bold" }}>Voice (TTS)</div>
          <div style={{ fontSize:11,color:T.textSub }}>{aiName} speaks her responses</div>
        </div>
        <button onClick={()=>setTtsOn(v=>{lsSave("echo_tts",!v);return !v;})} style={{ padding:"8px 16px",borderRadius:12,border:`1.5px solid ${ttsOn?pal.b+"66":T.border}`,background:ttsOn?`${pal.b}22`:T.inputBg,color:ttsOn?pal.b:T.textSub,fontSize:12,cursor:"pointer",boxShadow:`${T.shadow1},${T.shadow2}` }}>{ttsOn?"On":"Off"}</button>
      </div>

      {/* Image Mode */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:10,letterSpacing:1,textTransform:"uppercase" }}>Image Mode</div>
        <div style={{ display:"flex",gap:8,marginBottom:12 }}>
          {Object.keys(IMAGE_MODELS).map(m=>(
            <button key={m} onClick={()=>{setImgMode(m);lsSave("echo_imgmode",m);}}
              style={{ flex:1,padding:"9px 0",borderRadius:10,border:`1.5px solid ${imgMode===m?pal.b+"66":T.border}`,background:imgMode===m?`${pal.b}22`:T.inputBg,color:imgMode===m?pal.b:T.textSub,fontSize:12,cursor:"pointer",boxShadow:imgMode===m?`0 2px 12px ${pal.b}33`:`${T.shadow1},${T.shadow2}`,transition:"all 0.2s" }}>
              {m}
            </button>
          ))}
        </div>
        {!imgProUnlocked ? (
          <div>
            <p style={{ fontSize:11,color:T.textSub,marginBottom:8 }}>🔒 EchoImagen Pro — enter password</p>
            <div style={{ display:"flex",gap:8 }}>
              <input type="password" value={imgProPass} onChange={e=>setImgProPass(e.target.value)}
                placeholder="Password..." style={{ flex:1,padding:"8px 12px",borderRadius:10,border:`1.5px solid ${T.border}`,fontSize:13,outline:"none",background:T.inputBg,color:T.text,boxShadow:`inset ${T.shadow1}` }}/>
              <button onClick={()=>{if(imgProPass===PRO_PASSWORD)setImgProUnlocked(true);else alert("Wrong password 🌸");setImgProPass("");}}
                style={{ padding:"8px 14px",borderRadius:10,border:`1.5px solid ${pal.b}`,background:`${pal.b}18`,color:pal.b,fontSize:12,cursor:"pointer",boxShadow:`${T.shadow1},${T.shadow2}` }}>
                Unlock
              </button>
            </div>
          </div>
            ...
            <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
              {Object.keys(IMAGE_MODELS_PRO).map(m=>(
                <button key={m} onClick={()=>{const val=imgProModel===m?null:m;setImgProModel(val);lsSave("echo_imgpromodel",val);}}
                  style={{ padding:"7px 12px",borderRadius:10,border:`1.5px solid ${imgProModel===m?"#70c498":T.border}`,background:imgProModel===m?"#70c49822":T.inputBg,color:imgProModel===m?"#70c498":T.textSub,fontSize:11,cursor:"pointer",boxShadow:`${T.shadow1},${T.shadow2}` }}>
                  {m}
                </button>
              ))}
            </div>
      ...
      {/* Memories */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:6,letterSpacing:1,textTransform:"uppercase" }}>Memories ({memories.length})</div>
        <p style={{ fontSize:11,color:T.textSub,marginBottom:10 }}>Say "remember that..." or "forget..." in chat</p>
        {memories.length ? memories.map((m,i)=>(
          <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:T.inputBg,borderRadius:10,marginBottom:4,fontSize:12,color:T.text,boxShadow:`inset ${T.shadow1}`,border:`1px solid ${T.border}` }}>
            <span>{m}</span>
        ...
        )) : <p style={{ fontSize:11,color:T.textMuted,textAlign:"center" }}>No memories yet 🌸</p>}
      </div>

      {/* Chat history */}
      <div style={{ ...neuCard,padding:18,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div>
          <div style={{ fontSize:13,color:T.text,fontWeight:"bold" }}>Chat History</div>
          <div style={{ fontSize:11,color:T.textSub }}>{messages.length} messages</div>
        </div>
        <button onClick={()=>{setMessages([]);setHistory([]);lsSave("echo_msgs",[]);lsSave("echo_history",[]);}} style={{ padding:"8px 14px",borderRadius:12,border:`1.5px solid ${pal.b}`,background:`${pal.b}18`,color:pal.b,fontSize:12,cursor:"pointer",boxShadow:`${T.shadow1},${T.shadow2}` }}>Clear</button>
      </div>

      {/* Train */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:10,letterSpacing:1,textTransform:"uppercase" }}>Train {aiName}</div>
        {!unlocked ? (
          <div>
            <p style={{ fontSize:13,color:T.textSub,marginBottom:10 }}>Enter developer password</p>
            <input value={passIn} onChange={e=>setPassIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(passIn==="luminar"?(setUnlocked(true),setPassErr(false)):setPassErr(true))} type="password" placeholder="Password..." style={{ width:"100%",padding:"10px 14px",borderRadius:10,border:`1.5px solid ${passErr?pal.b:T.border}`,fontSize:14,color:T.text,marginBottom:8,outline:"none",background:T.inputBg,boxShadow:`inset ${T.shadow1}` }}/>
            {passErr&&<p style={{ fontSize:12,color:pal.b,marginBottom:8 }}>Wrong password</p>}
            <button onClick={()=>passIn==="luminar"?(setUnlocked(true),setPassErr(false)):setPassErr(true)} style={{ width:"100%",padding:"10px 0",borderRadius:12,background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:14,cursor:"pointer",boxShadow:`0 4px 16px ${pal.b}44` }}>Unlock</button>
            <p style={{ fontSize:11,color:T.textMuted,marginTop:8,textAlign:"center" }}>Default: luminar</p>
          </div>
        ) : (
          <div>
            <input value={trigIn} onChange={e=>setTrigIn(e.target.value)} placeholder="Triggers: hi, hello, hey" style={{ width:"100%",padding:"10px 14px",borderRadius:10,border:`1.5px solid ${T.border}`,fontSize:13,color:T.text,marginBottom:10,outline:"none",background:T.inputBg,boxShadow:`inset ${T.shadow1}` }}/>
            <textarea value={resIn} onChange={e=>setResIn(e.target.value)} placeholder="Responses: Hey!|Hello!|Heyy!" rows={3} style={{ width:"100%",padding:"10px 14px",borderRadius:10,border:`1.5px solid ${T.border}`,fontSize:13,color:T.text,resize:"none",marginBottom:10,outline:"none",background:T.inputBg,boxShadow:`inset ${T.shadow1}` }}/>
            <button onClick={train} style={{ width:"100%",padding:"12px 0",borderRadius:12,background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:15,...pf,cursor:"pointer",boxShadow:`0 4px 16px ${pal.b}44` }}>Train {aiName}</button>
            {trainLog.length>0&&(
              <div style={{ marginTop:14 }}>
                <button onClick={()=>setShowLog(v=>!v)} style={{ background:"none",border:"none",cursor:"pointer",fontSize:12,color:T.textSub,padding:0,marginBottom:8 }}>{showLog?"Hide":"View"} Log ({trainLog.length})</button>
                {showLog&&<div style={{ maxHeight:200,overflowY:"auto" }}>{[...trainLog].reverse().map((e,idx)=>(
                  <div key={e.id||idx} style={{ borderBottom:`1px solid ${T.border}`,padding:"6px 4px",fontSize:11,color:T.textSub,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8 }}>
                    <div style={{ flex:1 }}>
                      <span style={{ color:pal.b }}>{e.triggers}</span>{" → "}{e.responses.split("|").map((r,j)=><span key={j} style={{ display:"inline-block",background:T.inputBg,borderRadius:6,padding:"1px 6px",margin:"1px 2px",fontSize:10,color:T.text }}>{r.trim()}</span>)}
                    </div>
      ...
      {/* Data Packs */}
      <div style={{ ...neuCard,padding:18,marginBottom:14 }}>
        <div style={{ fontSize:12,color:T.textMuted,marginBottom:4,letterSpacing:1,textTransform:"uppercase" }}>Data Packs</div>
        <p style={{ fontSize:11,color:T.textSub,marginBottom:12 }}>Upload JSON training packs. Max 5MB.</p>
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex",justifyContent:"space-between",fontSize:11,color:T.textSub,marginBottom:4 }}><span>Used: {storageKb}KB</span><span>Max: 5120KB</span></div>
          <div style={{ height:6,borderRadius:4,background:T.inputBg,overflow:"hidden",boxShadow:`inset ${T.shadow1}` }}>
        ...
        <label style={{ display:"block",padding:"10px 0",borderRadius:12,textAlign:"center",background:`${pal.b}18`,border:`1.5px dashed ${pal.b}88`,cursor:"pointer",fontSize:13,color:pal.b,marginBottom:12 }}>
          Upload Pack (.json)<input type="file" accept=".json" onChange={uploadPack} style={{ display:"none" }}/>
        </label>
        {packs.length ? packs.map((p,i)=>(
          <div key={p.id||i} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",background:T.inputBg,borderRadius:10,marginBottom:6,border:`1px solid ${T.border}`,boxShadow:`inset ${T.shadow1}` }}>
            <div>
              <div style={{ fontSize:12,color:T.text }}>{p.name}</div>
              <div style={{ fontSize:10,color:T.textSub }}>{p.entries} entries · {p.size}</div>
            </div>
            <button onClick={()=>delPack(i)} style={{ background:"none",border:"none",cursor:"pointer" }}><IcoTrash/></button>
          </div>
        )) : <p style={{ fontSize:11,color:T.textMuted,textAlign:"center" }}>No packs loaded yet</p>}
      </div>

      <p style={{ textAlign:"center",fontSize:11,color:T.textMuted,letterSpacing:2,marginTop:10,paddingBottom:20 }}>{aiName.toUpperCase()} BY LUMINAR INC</p>
    </div>
  );
  if (screen === "apikeys") {
  ...
  const next = activeImagen === id ? null : id;
  setActiveImagen(next); lsSave("echo_active_imagen", next);
};
  ...
  const KeyRow = ({ provider, cat, isRadio, activeId, onToggle }) => {
    ...
    return (
      <div style={{ background:T.inputBg,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`1px solid ${T.border}`,boxShadow:`inset ${T.shadow1}` }}>
        <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8 }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:13,fontWeight:700,color:T.text }}>{provider.label}</div>
            <div style={{ fontSize:10,color:T.textSub }}>{provider.company} · {provider.free}</div>
            {provider.mode && (
            ...
            <>
              <button onClick={()=>onToggle(provider.id)}
                style={{ padding:"5px 12px",borderRadius:20,border:"none",cursor:"pointer",
                  ...
                  background:isOn?`linear-gradient(135deg,${pal.b},${pal.c})`:T.surface,
                  color:isOn?"white":T.textSub,display:"flex",alignItems:"center",gap:5,
                  boxShadow:`${T.shadow1},${T.shadow2}` }}>
                {isOn && (
              ...
              <button onClick={()=>deleteKey(cat,provider.id)}
                ...
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={pal.b} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {hasKey ? (
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <div style={{ flex:1,padding:"7px 12px",borderRadius:10,background:T.surface,
              border:`1px solid ${T.border}`,fontSize:12,color:T.textSub,
              fontFamily:"monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
              boxShadow:`${T.shadow1},${T.shadow2}` }}>
              {show ? val : "•".repeat(Math.min(val.length,32))}
            </div>
            <button onClick={()=>setShow(v=>!v)}
              style={{ padding:"7px 10px",borderRadius:10,background:T.surface,
                border:`1px solid ${T.border}`,cursor:"pointer",fontSize:11,color:T.textSub,
                boxShadow:`${T.shadow1},${T.shadow2}` }}>
              {show ? "Hide" : "Show"}
            </button>
          </div>
          ...
          <div style={{ display:"flex",gap:8 }}>
            <input
              ...
              style={{ flex:1,padding:"9px 12px",borderRadius:10,
                ...
                outline:"none",background:T.surface,boxShadow:`inset ${T.shadow1}` }}
            />
            <button onClick={()=>{ if(draft.trim()){ saveKey(cat,provider.id,draft.trim()); setDraft(""); }}}
              style={{ padding:"9px 14px",borderRadius:10,border:"none",cursor:"pointer",
                ...
                color:"white",fontSize:12,fontWeight:700,boxShadow:`0 4px 12px ${pal.b}44` }}>
              Save
            </button>
          </div>
        )}
  ...
  const SectionCard = ({ title, icon, accent, children }) => {
    ...
    return (
      <div style={{ ...neuCard,marginBottom:14,overflow:"hidden" }}>
        <button onClick={()=>setOpen(v=>!v)}
          ...
          <div style={{ flex:1,textAlign:"left" }}>
            <div style={{ fontSize:14,fontWeight:700,color:T.text,...pf }}>{title}</div>
          </div>
  ...
  return (
    <div style={{ height:"100vh",overflowY:"auto",...BG,padding:"24px 20px",...lf }}>
      <style>{GF}</style>
      <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:24 }}>
        <button onClick={()=>setScreen("chat")}
          style={{ background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:6,color:T.textSub,fontSize:14 }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.textSub} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <div style={{ flex:1 }}>
          <h2 style={{ ...pf,fontSize:24,color:T.text,margin:0 }}>API Hub</h2>
          <p style={{ fontSize:11,color:T.textMuted,margin:0,letterSpacing:1 }}>BRING YOUR OWN KEY</p>
        </div>
        <ThemeToggle/>
      </div>
      {(chatActive || enhanceActive) && (
        <div style={{ background:`linear-gradient(135deg,${pal.b}18,${pal.c}14)`,borderRadius:14,
          padding:"12px 16px",marginBottom:16,border:`1px solid ${pal.b}33`,boxShadow:`${T.shadow1},${T.shadow2}` }}>
          <div style={{ fontSize:11,color:pal.b,fontWeight:700,letterSpacing:1,marginBottom:6 }}>ACTIVE OVERRIDES</div>
          {chatActive && <div style={{ fontSize:12,color:T.text,marginBottom:2 }}>Chat → {chatActive.label}</div>}
          {enhanceActive && <div style={{ fontSize:12,color:T.text }}>Enhance → {enhanceActive.label}</div>}
        </div>
      )}
      <SectionCard title="Chat APIs" accent={pal.b} icon={
        ...
        <p style={{ fontSize:11,color:T.textSub,marginBottom:12,lineHeight:1.6 }}>One active at a time. Overrides Echo's default chat endpoint.</p>
      ...
      <SectionCard title="Image Generation APIs" accent="#70c498" icon={
        ...
        <p style={{ fontSize:11,color:T.textSub,marginBottom:12,lineHeight:1.6 }}>Keys auto-activate for their assigned mode when you generate images.</p>
        {IMAGEN_PROVIDERS.map(p=>(
          <KeyRow key={p.id} provider={p} cat="imagen" isRadio={false} activeId={null} onToggle={toggleImagen}/>
        ))}
        <div style={{ background:T.inputBg,borderRadius:10,padding:"10px 12px",fontSize:11,color:T.textSub,lineHeight:1.6,border:`1px solid ${T.border}` }}>
      ...
      <SectionCard title="Image Enhancer APIs" accent="#c4a8d4" icon={
        ...
        <p style={{ fontSize:11,color:T.textSub,marginBottom:12,lineHeight:1.6 }}>Active when you tap Enhance in the + menu. One active at a time.</p>
        {ENHANCE_PROVIDERS.map(p=>(
          <KeyRow key={p.id} provider={p} cat="enhance" isRadio activeId={activeEnhance} onToggle={toggleEnhance}/>
        ))}
      </SectionCard>
      <p style={{ textAlign:"center",fontSize:10,color:T.textMuted,letterSpacing:2,marginTop:8,paddingBottom:24 }}>
