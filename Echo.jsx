      ...
      <div ref={chatRef} onScroll={handleScroll} style={{ flex:1,overflowY:"auto",padding:"16px 12px",display:"flex",flexDirection:"column",gap:8,position:"relative" }}>
        {messages.map((msg,i)=>(
          ...
          >
            ...
            {msg.ts && <div style={{ fontSize:9,color:"#c4b4a8",marginTop:2,marginLeft:msg.f==="u"?0:4,marginRight:msg.f==="u"?4:0 }}>{formatTime(msg.ts)}</div>}
            {longPressId === msg.id && (
              <div style={{ background:"white",border:"1px solid #f0e4d4",borderRadius:10,padding:"6px 12px",boxShadow:"0 4px 16px #00000018",display:"flex",alignItems:"center",gap:8,marginTop:4,cursor:"pointer",zIndex:100 }}
                onClick={()=>copyMsg(msg.id, msg.t)}>
                <IcoCopy/>
                <span style={{ fontSize:12,color:"#6b5040" }}>{copiedId===msg.id ? "Copied!" : "Copy"}</span>
              </div>
            )}
            {msg.f==="e"&&!msg.sys&&(
