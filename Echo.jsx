...
async function webSearch(query) {
  try {
    ...
    if (parsed.RelatedTopics) {
      parsed.RelatedTopics.slice(0,5).forEach(t => { if (t.Text) results.push(t.Text); });
    }
...
async function webFetch(url) {
  try {
    const res = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`);
    const data = await res.json();
    const clean = data.contents.replace(/<[^>]+>/g,"").replace(/\s+/g," ").trim().slice(0,3000);
    return clean || null;
  } catch { return null; }
}
...
function Dots({ color }) {
  return (
    <div style={{ display:"flex",gap:5,alignItems:"center",padding:"12px 16px" }}>
      {[0,1,2].map(i=>(
        <div key={i} style={{ width:7,height:7,borderRadius:"50%",background:color,animation:`dotB 1s ease-in-out ${i*0.15}s infinite` }}/>
      ))}
    </div>
  );
}
...
useEffect(() => { endRef.current?.scrollIntoView({ behavior:"smooth" }); }, [messages, thinking]);
  ...
  const send = async () => {
    ...
    if (reply) {
      try {
        ...
        if (action.action === "search" && action.query) {
          ...
          reply = await askAI(txt, sys, history, webCtx || "No search results found.");
        } else if (action.action === "fetch" && action.url) {
          const webCtx = await webFetch(action.url);
          reply = await askAI(txt, sys, history, webCtx || "Could not fetch that page.");
        }
      } catch {}
    }
    const raw = reply || `I'm having a little trouble connecting right now. Try again in a moment 🌸`;
    ...
    pushHist(newH);
    const newM = [...withUser,{f:"e",t:final}];
    pushMsgs(newM); setThinking(false); setOrbState("idle");
    tts(final, ttsOn);
  };

  const like = (i, msg) => {
    if (liked.has(i)||disliked.has(i)) return;
    const u = new Set([...liked,i]); setLiked(u); lsSave("echo_liked",u);
    const td = [...trainedData,{triggers:[],responses:[msg.t]}];
    setTrainedData(td); lsSave("echo_trained",td);
  };
  const dislike = (i, msg) => {
    if (liked.has(i)||disliked.has(i)) return;
    const u = new Set([...disliked,i]); setDisliked(u); lsSave("echo_disliked",u);
    const bl = new Set([...blacklist,msg.t]); setBlacklist(bl); lsSave("echo_bl",bl);
  };
  const train = () => {
...
<div style={{ padding:"8px 14px",background:"rgba(253,246,238,0.95)",borderBottom:"1px solid #f0e4d4",display:"flex",gap:6 }}>
          ...
          <div key={i} style={{ display:"flex",flexDirection:"column",alignItems:msg.f==="u"?"flex-end":"flex-start" }}>
            ...
            {msg.f==="e"&&!msg.sys&&(
              <div style={{ display:"flex",gap:6,marginTop:4,marginLeft:4,alignItems:"center" }}>
                <button onClick={()=>like(i,msg)} disabled={liked.has(i)||disliked.has(i)} style={{ background:liked.has(i)?"#e8f5e8":"none",border:`1px solid ${liked.has(i)?"#6ab06a":"#c4e8c4"}`,borderRadius:10,padding:"2px 8px",cursor:(liked.has(i)||disliked.has(i))?"default":"pointer",display:"flex",alignItems:"center",opacity:disliked.has(i)?0.3:1,transition:"all 0.2s" }}>
                  <IcoThumbUp on={liked.has(i)}/>
                </button>
                <button onClick={()=>dislike(i,msg)} disabled={liked.has(i)||disliked.has(i)} style={{ background:disliked.has(i)?"#fce8e8":"none",border:`1px solid ${disliked.has(i)?"#c46a6a":"#f0c4c4"}`,borderRadius:10,padding:"2px 8px",cursor:(liked.has(i)||disliked.has(i))?"default":"pointer",display:"flex",alignItems:"center",opacity:liked.has(i)?0.3:1,transition:"all 0.2s" }}>
                  <IcoThumbDown on={disliked.has(i)}/>
                </button>
              </div>
            )}
          </div>
  ...
  if (screen === "settings") return (
    <div style={{ height:"100vh", overflowY:"auto", background:"linear-gradient(160deg,#fdf6ee,#f5e4f0)", padding:24, ...lato }}>
      ...
      <p style={{ textAlign:"center",fontSize:11,color:"#c4c4c4",letterSpacing:2,marginTop:10 }}>{aiName.toUpperCase()} BY LUMINAR INC</p>
    </div>
  );
}