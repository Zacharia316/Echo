...
export default function EchoApp() {
  ...
  try {
    ...
    if (imgMode === "Fast") {
      ...
      if (!data.success || !data.image_url) throw new Error("No image");
  ...
  if (screen==="chat") return (
    <div style={{height:"100vh",background:C.bg,display:"flex",flexDirection:"column",...font}}>
      ...
      <div ref={chatBox} onScroll={()=>{const el=chatBox.current;if(el)setShowScroll(el.scrollHeight-el.scrollTop-el.clientHeight>120);}} style={{flex:1,overflowY:"auto",padding:"14px 12px",display:"flex",flexDirection:"column",gap:8,position:"relative"}}>
        {messages.map((msg,i)=>{
      ...
      {showScroll&&(
        <button onClick={()=>{msgEnd.current?.scrollIntoView({behavior:"smooth"});setShowScroll(false);}} style={{position:"fixed",bottom:90,right:16,zIndex:40,width:38,height:38,borderRadius:"50%",background:C.surface,border:`1px solid ${C.border}`,color:C.text,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(0,0,0,.5)"}}>
