...
async function askAI(userText, systemPrompt, history, webContext) {
  try {
    ...
    const res = await fetch(AI_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ model:"claude", messages, token: POLLINATIONS_KEY }),
    });
    if (!res.ok) throw new Error();
    const data = await res.json();
...
const generateImage = async (prompt) => {
  const model = imgProUnlocked ? IMAGE_MODELS_PRO[imgProModel] : IMAGE_MODELS[imgMode];
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=${model}&token=${POLLINATIONS_KEY}&nologo=true`;
  const imgMsg = {f:"e", src:"ai", id:Date.now(), t:`🎨 Here's your image!`, imgUrl:url};
    ...
    <div>
      ...
      <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
        {Object.keys(IMAGE_MODELS_PRO).map(m=>(
          <button key={m} onClick={()=>{setImgProModel(m);lsSave("echo_imgpromodel",m);}}
