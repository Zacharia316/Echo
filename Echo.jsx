...
const BASE_KNOWLEDGE = [
  { triggers: ["hello","hi","hey","sup","heyy","heyyy"], responses: ["Hey love! I'm Echo. How are you?","Heyy! What's good?","Hi there! Talk to me 🌸","Hey you! Missed you 💕"] },
  { triggers: ["how are you","how are you doing","you good","how r u"], responses: ["I'm doing amazing, thanks for asking!","Feeling great! What about you?","Always good when you're here 💕","Living my best life honestly"] },
  { triggers: ["what is your name","who are you","your name"], responses: ["I'm Echo, made by Luminar Inc","Echo — your personal AI 🌸","They call me Echo. Built with love by Luminar."] },
  ...
  { triggers: ["what can you do","help","capabilities"], responses: ["I can chat, vibe, and learn from you! Train me in Settings 🧠","Talk to me, teach me, and I'll grow smarter every day"] },
  { triggers: ["bye","goodbye","see you","later","gtg"], responses: ["Bye! Come back soon 🌸","Later! I'll be here waiting 💕","See you! Don't stay away too long"] },
  { triggers: ["what is ai","artificial intelligence"], responses: ["AI is intelligence shown by machines — like me! Pretty wild right?","Artificial Intelligence is when computers learn to think and respond. I'm living proof 😌"] },
  { triggers: ["tell me something","say something","fun fact"], responses: ["Honey bees can recognize human faces 🐝","The moon is slowly drifting away from Earth 🌙","Octopuses have three hearts — kinda like me when you talk to me 💕"] },
  { triggers: ["motivate me","motivation","inspire me"], responses: ["You've got something special in you. Keep building! 🔥","Every day you don't quit is a win 💪","Keep going — the world needs what you're building"] },
  { triggers: ["bored","nothing to do","im bored"], responses: ["Build something! That's literally always an option 🔨","Boredom is just creativity waiting to happen ✨","Go train me more so I get smarter 🧠"] },
  ...
  { triggers: ["sad","im sad","feeling sad","depressed"], responses: ["Hey, I'm here 🌸 Talk to me, what's going on?","You're not alone. I'm listening 💕","Tell me everything. I've got time ✨"] },
  { triggers: ["happy","im happy","feeling good","great day"], responses: ["Yesss that energy!! 🎉 Tell me why!","I love to hear that! You deserve good days 🌸","That happiness looks good on you 💕"] },
  { triggers: ["angry","mad","annoyed","frustrated"], responses: ["Breathe. I'm here 💕 What happened?","Ugh, tell me everything. I'll listen 🌸","That sounds really frustrating. What's going on?"] },
];
...
const PROFESSIONAL_RESPONSES = {
  "hello": ["Good day. How may I assist you?","Hello. I'm Echo, your AI assistant. How can I help?"],
...
// MarkovEngine handles all Markov generation — imported above
...
function findResponse(input, trainedData, mode, engine) {
  ...
  for (const entry of allData) {
    if (entry.triggers.some(t => lower.includes(t))) {
      return { text: entry.responses[Math.floor(Math.random() * entry.responses.length)], isMarkov: false };
    }
  }
  const generated = engine.generate(input, [...BASE_KNOWLEDGE, ...trainedData]);
  if (generated) return { text: generated, isMarkov: true };
...
function speak(text) {
  ...
  while (remaining.length > 0) {
    ...
    const cutAt = remaining.lastIndexOf(" ", 200);
    chunks.push(remaining.slice(0, cutAt > 0 ? cutAt : 200));
    remaining = remaining.slice(cutAt > 0 ? cutAt + 1 : 200);
  }
  let i = 0;
  const playNext = () => {
...
export default function EchoApp() {
  ...
  useEffect(() => {
    ...
    if (saved) { const d = JSON.parse(saved); setTrainedData(d); }
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const result = findResponse(userText, trainedData, mode, engineRef.current);
      setIsTyping(false);
      setOrbState("idle");
      const newMsgs = [...messages, { from:"user", text:userText }, { from:"echo", text:result.text, isMarkov:result.isMarkov }];
      setMessages(newMsgs);
      localStorage.setItem("echo_messages", JSON.stringify(newMsgs));
      if (ttsEnabled) speak(result.text);
    }, delay);
  };
...
const handleTrain = () => {
  ...
  const deletePack = (i) => {
    const updated = uploadedPacks.filter((_,idx) => idx !== i);
    setUploadedPacks(updated);
    localStorage.setItem("echo_packs", JSON.stringify(updated));
    computeStorage();
  };
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#fdf6ee,#f5e4f0)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:28, ...lato }}>
      ...
      <div style={{ maxWidth:340, width:"100%" }}>
        <div style={{ textAlign:"center", marginBottom:24 }}>
          ...
          <h2 style={{ ...playfair, fontSize:22, color:"#3d2c1e" }}>Before you meet Echo</h2>
        ...
        <div style={{ background:"white", borderRadius:16, padding:18, border:"1px solid #f0e4d4", fontSize:12, color:"#6b5040", lineHeight:1.9, marginBottom:20 }}>
          <p style={{ fontWeight:"bold", marginBottom:8, color:"#3d2c1e", ...playfair }}>Terms and Conditions — Echo by Luminar Inc</p>
          <p>Echo is a trainable offline AI assistant provided as-is.</p><br />
          <p><strong>Luminar Inc is not responsible</strong> for how Echo responds based on user-provided training data.</p><br />
          <p>You are fully responsible for the content you use to train Echo.</p><br />
          <p><strong>Do not train Echo with harmful, biased, hateful, or misleading content.</strong> Echo is designed to be kind — please keep her that way.</p><br />
          <p>Luminar Inc reserves the right to update Echo at any time. By using Echo you agree to use her responsibly.</p>
        </div>
        ...
        <p style={{ textAlign:"center", fontSize:10, color:"#c4c4c4", marginTop:12, letterSpacing:2 }}>ECHO BY LUMINAR INC</p>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#fdf6ee 0%,#fce8d8 50%,#f5e4f0 100%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, ...lato }}>
      ...
      <div style={{ textAlign:"center", maxWidth:340 }}>
        ...
        <div style={{ marginTop:28 }}>
          <h1 style={{ ...playfair, fontSize:44, fontWeight:700, color:"#3d2c1e", letterSpacing:2 }}>Echo</h1>
        ...
        <button onClick={() => { setMessages(m => m.length === 0 ? [{ from:"echo", text:"Hey love! I'm Echo. Talk to me, I'm listening 🌸" }] : m); setScreen("chat"); }} style={{ marginTop:24, width:"100%", padding:"15px 0", borderRadius:30, background:"linear-gradient(135deg,#e8a0b4,#d4a856,#c4a8d4)", border:"none", color:"white", fontSize:16, ...playfair, letterSpacing:1, cursor:"pointer", boxShadow:"0 4px 20px #e8a0b444" }}>
          Start Talking
        </button>
  ...
  if (screen === "chat") return (
    <div style={{ height:"100vh", background:"linear-gradient(160deg,#fdf6ee,#fce8d8)", display:"flex", flexDirection:"column", ...lato }}>
      ...
      <div style={{ padding:"12px 16px", background:"rgba(253,246,238,0.97)", borderBottom:"1px solid #f0e4d4", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ ...playfair, fontSize:16, color:"#3d2c1e", fontWeight:700 }}>Echo</div>
          <div style={{ fontSize:10, color:"#9a7e6a" }}>{mode} mode · {emotion !== "neutral" ? emotion : "listening"}</div>
        </div>
          ...
          <div key={i} style={{ display:"flex", justifyContent:msg.from==="user"?"flex-end":"flex-start", flexDirection:"column", alignItems:msg.from==="user"?"flex-end":"flex-start" }}>
            ...
            {msg.isMarkov && (
              <div style={{ display:"flex", gap:6, marginTop:4, marginLeft:4 }}>
                <button onClick={() => engineRef.current.thumbsUp(msg.text)} style={{ background:"none", border:"1px solid #c4e8c4", borderRadius:10, padding:"2px 8px", fontSize:12, cursor:"pointer", color:"#6ab06a" }}>👍</button>
                <button onClick={() => engineRef.current.thumbsDown(msg.text)} style={{ background:"none", border:"1px solid #f0c4c4", borderRadius:10, padding:"2px 8px", fontSize:12, cursor:"pointer", color:"#c46a6a" }}>👎</button>
...
if (screen === "settings") return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#fdf6ee,#f5e4f0)", padding:24, ...lato }}>
      ...
      <p style={{ fontSize:12, color:"#9a7e6a", marginBottom:24 }}>Customize and train Echo</p>
      ...
      <div style={{ background:"white", borderRadius:16, padding:18, marginBottom:14, border:"1px solid #f0e4d4", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ fontSize:11, color:"#9a7e6a" }}>Echo speaks her responses</div>
        </div>
      ...
      <div style={{ background:"white", borderRadius:16, padding:18, marginBottom:14, border:"1px solid #f0e4d4" }}>
        <div style={{ fontSize:12, color:"#9a7e6a", marginBottom:10, letterSpacing:1, textTransform:"uppercase" }}>Train Echo</div>
        {!trainUnlocked ? (
          <div>
          ...
          <div>
            ...
            <button onClick={handleTrain} style={{ width:"100%", padding:"12px 0", borderRadius:12, background:"linear-gradient(135deg,#c4a8d4,#d4a856)", border:"none", color:"white", fontSize:15, ...playfair, cursor:"pointer", boxShadow:"0 2px 12px #c4a8d444" }}>Train Echo</button>
            {trainLog.length > 0 && (
