...
const GROQ_KEY = "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";
...
function buildSystem(name, mode, userName, memories, trainedData, emotion) {
 ...
 s += `\n\nWEB SEARCH ABILITY: You have the ability to search the web and fetch URLs. Use it proactively.\n\nAUTO-SEARCH RULES — search automatically (without being asked) when the user asks about:\n- Current events, news, or anything recent\n- Prices, exchange rates, or market data\n- Weather\n- Who currently holds a position (president, CEO, etc)\n- Sports scores or results\n- Anything where your training data might be outdated\n\nTo search: respond with exactly this JSON on the first line only: {"action":"search","query":"<terms>"}\nTo fetch a URL: {"action":"fetch","url":"<url>"}\n\nCRITICAL: ALWAYS trust web search results over your training data. Never contradict or second-guess search results with your own assumptions.`;
  return s;
}
...
const [imgProModel, setImgProModel] = useState(ls("echo_imgpromodel","GPT Image 1.5"));
...
const generateImage = async (prompt) => {
  ...
  const data = await res.json();
  console.log("Keith image response:", JSON.stringify(data));
  imgUrl = data?.result || data?.url || data?.image || data?.data;
    ...
    const imgMsg = { f:"e", src:"ai", id:Date.now(), t:`🎨 Here's your image! (${imgProUnlocked ? imgProModel : imgMode})`, imgUrl };
    setMessages(prev => { const u = [...prev, imgMsg]; lsSave("echo_msgs",u); return u; });
  } catch (err) {
    const errMsg = { f:"e", src:"ai", id:Date.now(), t:`❌ Image generation failed. Try another model 🌸` };
...
if (imageKeywords.some(k => txt.toLowerCase().includes(k))) {
  ...
  if (jsonStart !== -1) {
    ...
    try { action = JSON.parse(reply.slice(jsonStart, jsonEnd)); } catch {}
  }
  if (action?.action === "search" && action.query) {
    ...
    pushHist(newH);
    const newM = [...withUser,{f:"e",t:final,id:Date.now(),src:reply?"ai":"markov"}];
...
const uploadPack = (e) => {
  ...
  if (screen === "chat") return (
    <div style={{ height:"100vh",background:"linear-gradient(160deg,#fdf6ee,#fce8d8)",display:"flex",flexDirection:"column",position:"relative",...lf }}>
      ...
      <div ref={chatRef} onScroll={handleScroll} style={{ flex:1,overflowY:"auto",padding:"16px 12px",display:"flex",flexDirection:"column",gap:8,position:"relative" }}>
          <div key={i} style={{ display:"flex",flexDirection:"column",alignItems:msg.f==="u"?"flex-end":"flex-start" }}>
            <div style={{ maxWidth:"78%",padding:"11px 15px",borderRadius:msg.f==="u"?"20px 20px 4px 20px":"20px 20px 20px 4px",background:msg.f==="u"?`linear-gradient(135deg,${pal.b},${pal.c})`:"rgba(255,255,255,0.95)",color:msg.f==="u"?"white":"#3d2c1e",fontSize:14,lineHeight:1.65,boxShadow:msg.f==="u"?`0 2px 12px ${pal.b}44`:"0 2px 8px #00000011",border:msg.f==="e"?"1px solid #f0e4d4":"none",whiteSpace:"pre-line" }}>
              {!msg.sys && (
  <div style={{ fontSize:9,color:msg.src==="markov"?"#c4a8d4":"#e8a0b4",letterSpacing:1.5,marginBottom:4,textTransform:"uppercase" }}>
  ...
  <IcoThumbDown on={disliked.has(msg.id)}/>
      ...
      {showScrollBtn && (
        <button onClick={()=>endRef.current?.scrollIntoView({behavior:"smooth"})}
          style={{ position:"absolute",bottom:80,right:16,width:38,height:38,borderRadius:"50%",background:`linear-gradient(135deg,${pal.b},${pal.c})`,border:"none",color:"white",fontSize:18,cursor:"pointer",boxShadow:`0 2px 12px ${pal.b}66`,zIndex:10 }}>
          ↓
        </button>
      )}
  ...
  <div style={{ display:"flex",gap:8,marginBottom:12 }}>
    {Object.keys(IMAGE_MODELS).map(m=>(
      <button key={m} onClick={()=>{setImgMode(m);lsSave("echo_imgmode",m);}}
        style={{ flex:1,padding:"9px 0",borderRadius:10,border:`1.5px solid ${imgMode===m?"#e8a0b4":"#f0e4d4"}`,background:imgMode===m?"#e8a0b418":"white",color:imgMode===m?"#e8a0b4":"#9a7e6a",fontSize:12,cursor:"pointer" }}>
        {m}
      </button>
    ))}
  </div>
    ...
    <div>
      ...
      <div style={{ display:"flex",gap:8 }}>
        ...
        <button onClick={()=>{if(imgProPass===PRO_PASSWORD)setImgProUnlocked(true);else alert("Wrong password 🌸");setImgProPass("");}}
          style={{ padding:"8px 14px",borderRadius:10,border:"1.5px solid #e8a0b4",background:"#e8a0b418",color:"#e8a0b4",fontSize:12,cursor:"pointer" }}>
          Unlock
        </button>
      </div>
    </div>
